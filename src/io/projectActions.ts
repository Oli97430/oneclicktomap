...
/** Sérialise l'état courant (projet + scènes + réglages) et l'enregistre via le dialogue natif. */
export async function saveProjectToFile(): Promise<void> {
  ...
  if (result && !result.ok && result.error) {
    window.alert(`Échec de l'enregistrement : ${result.error}`);
  }
}
...
export async function openProjectFromFile(): Promise<void> {
  ...
  if (!result || !result.ok || !result.text) {
    if (result?.error) window.alert(`Échec de l'ouverture : ${result.error}`);
    return;
  }
  try {
